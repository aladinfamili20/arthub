...
const [product, setProduct] = useState({
  ...
  const storageRef = ref(storage, `/diamond/${image.name}`);      
...
//   console.log(url);
  ...
  const addProduct = (e) => {
    ...
    try {
      const docRef = addDoc(collection(db, "diamond"), {
        name: product.name,
        image:image,
        price: Number(price),
  ...
  return (
    <>
        <section>
            ...
            <input type='number' placeholder='Price, 397' value={price} 
